...
const titleItem=["1","2","3","4"]
const timeItem=["2013","2014","2015","2016"]
const descriptionItem=["yigedasdfsafdasf","dfsfsfdsfs","fsdfdsfdd","dfsfdsfsddfs"]
// 图片展示项最大数量
const imgItemMaxCnt = 5
// 每次生成的图片展示项个数
 ...
 * @return 图片的宽高组成的对象
 */
const genRandImgSize = (min) => {
    return {
        // width: 1,
        height: genRandInt(min)
    }
}
...
const genImgItems = () => {
    ...
    for (let i = 0; i < imgItemCreateCntPer; i++) {
        ...
        // 生成随机图片地址
        let imgSize = genRandImgSize(pic)
        ...
        // let imgSrc = `https://picsum.photos/${imgSize.width}/${imgSize.height}`
		    let imgSrc = `./images/${imgSize.height}`+'.jpg'
        // 修改元素图片地址
		...
		title.textContent=titleItem[pic]
		
		//修改时间属性值
		let time = imgItem_clone.querySelector('.time')
		time.textContent=timeItem[pic]
		
		//修改详情属性值
		let description = imgItem_clone.querySelector('.description')
		description.textContent=descriptionItem[pic]
